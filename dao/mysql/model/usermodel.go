...
type (
	...
	UserModel interface {
		...
		FindList(ctx context.Context, ex goqu.Expression, optionalParams ...any) (*[]User, error)
		// FindOnly 根据条件获取单条数据，0-升序(ASC)；1-降序(DESC)
		FindOnly(ctx context.Context, ex goqu.Expression, order ...map[string]int) (*User, error)
		// InsertOnly 插入单条数据
		InsertOnly(ctx context.Context, row *User, tx ...*sql.Tx) (sql.Result, error)
		// BatchInsert 批量插入
		BatchInsert(ctx context.Context, rows []*User, tx ...*sql.Tx) (sql.Result, error)
		// UpdateByEx 根据条件更新
...
// GetTableName 获取表名
func (m *customUserModel) GetTableName() string {
	return utils.SetTable(m.table)
}

// GetCount 根据条件获取数量
func (m *customUserModel) GetCount(ctx context.Context, ex goqu.Expression) (int64, error) {
...
// FindList 根据条件获取列表，排序：map[string]int{"字段":0/1(0-升序(ASC)；1-降序(DESC))}；分页：[]uint{页码，每页条数}
func (m *customUserModel) FindList(ctx context.Context, ex goqu.Expression, optionalParams ...any) (*[]User, error) {
	sql := goqu.Dialect("mysql").Select(&User{}).From(utils.SetTable(m.table)).Where(ex)
...
// FindOnly 根据条件获取单条数据，0-升序(ASC)；1-降序(DESC)
func (m *customUserModel) FindOnly(ctx context.Context, ex goqu.Expression, order ...map[string]int) (*User, error) {
	sql := goqu.Dialect("mysql").Select(&User{}).From(utils.SetTable(m.table)).Where(ex)
	if len(order) > 0 {
...
// InsertOnly 插入单条数据
func (m *customUserModel) InsertOnly(ctx context.Context, row *User, tx ...*sql.Tx) (sql.Result, error) {
...
// BatchInsert 批量插入
func (m *customUserModel) BatchInsert(ctx context.Context, rows []*User, tx ...*sql.Tx) (sql.Result, error) {
...
// UpdateByEx 根据条件更新
func (m *customUserModel) UpdateByEx(ctx context.Context, record goqu.Record, ex goqu.Expression, tx ...*sql.Tx) (sql.Result, error) {
...
// DeleteByEx 根据条件删除数据
func (m *customUserModel) DeleteByEx(ctx context.Context, ex goqu.Expression, tx ...*sql.Tx) (sql.Result, error) {
