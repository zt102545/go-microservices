...
import (
	...
	"go-microservices/service/adminService/internal/handler"
	"go-microservices/service/adminService/internal/svc"
	"os"
	"strings"
)

var configFile = flag.String("f", "./service/adminService/etc/admin.yaml", "the config file")
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		err := c.NewConfigFromEtcd(etcd, "adminConfig")
		if err != nil {
			panic(err)
		}
	} else {
